...
T = 30  # Keep Constant
scaling_factor = 5e-9  # Vary accordingly
...
spring_radius = 2.5
spring_stiffness_constant = 50000
...
def ModifiedMidpoint_update():
    set_deriv_phase_space(phase_space, k1)

    for i in ti.static(range(num_particles)):
        for j in ti.static(range(DIM * 2)):
            k2[i][j] = phase_space[i][j] + (2 / 3) * h * k1[i][j]

    set_deriv_phase_space(k2, k3)

    for i in ti.static(range(num_particles)):
        for j in ti.static(range(DIM * 2)):
            phase_space[i][j] = phase_space[i][j] + h * k1[i][j]
...
def Midpoint_update():
    ...
    for i in ti.static(range(num_particles)):
        for j in ti.static(range(DIM * 2)):
            k2[i][j] = phase_space[i][j] + h / 2 * k1[i][j]
