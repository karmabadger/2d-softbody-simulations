...
T = 30  # Keep Constant
scaling_factor = 1e-7 # Vary accordingly
...
@ti.pyfunc
def get_area_boundingbox() -> ti.i32:
...
def get_center():
    ...
    return ti.Vector([(min_x + max_x) / 2, (min_y + max_y) / 2])


@ti.pyfunc
def get_area_triangulation() -> ti.f32:
    min_x = float('inf')
    max_x = float('-inf')
    min_y = float('inf')
    max_y = float('-inf')
