...
n_springs = gf.number_of_springs_estimator(g_rows, g_cols)


particles_grid = gf.grid_maker(g_rows, g_cols)
springs = gf.springs_gen(particles_grid, g_rows, g_cols, 2, 2, 500000, 100)
...
# print("nsprings: ", n_springs, ", real: ", len(springs))
...
#
gf.set_positions(particles_grid, g_rows, g_cols, 0.5, 0.7, 0.01, 0.01)
...
def redraw_all(gui):
    if spring_debug:
        for i in ti.static(range(len(particles))):
            # print(particles[i].position[0].x, ", ", particles[i].position[0].y)
